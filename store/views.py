...
def ShowPdfAfterCheckout(request,id,mylist):
    ...
    c.drawString(120, 780,"TRENDSU INVOICE AS PDF")

    date1 = object.created_at
    date1str = date1.strftime("%d %B %Y, %H:%M")
    c.drawString(20, 700,"Order Date:   " + date1str )

    c.drawString(20, 680,"User:   " + str(object.ordered_by))
    c.drawString(20, 660,"Total price:   " + str(object.total))

    c.drawString(20, 600,"Date:   " + str(object.created_at))
    c.drawString(20, 580,"Shipping Address:   " + str(object.shipping_address))
...
def SendEmail(request, id):
    ...
    if subject and message and to_email:
        try:
            ...
            my_file = ShowPdfAfterCheckout(request,id,mylist)

            msg.attach('generated.pdf', my_file, 'application/pdf')
...
class AdminPendingRefundRequestsView(ProdAdminRequiredMixin,TemplateView):
    template_name = "salesmanagers/adminPendingRefundRequests.html"
...
class AdminAllRefundRequestsView(ProdAdminRequiredMixin,TemplateView):
    template_name = "salesmanagers/adminAllRefundRequests.html"
...
class GrantRefundView(ProdAdminRequiredMixin,TemplateView):
    template_name = "salesmanagers/grant_refund.html"
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        dis = self.request.user.productmanagersadmin.distributor
        context["distributor"] = dis
...
class AdminSalesOrderDetailView(ProdAdminRequiredMixin,DetailView):
    template_name = "salesmanagers/adminorderDetailSales.html"
    model = OrderProduct
    context_object_name = "ord_obj"
